...
def results(params,disks_f,R_d,dR_d,dic,galaxy,fix_arccos=False,warp_fit=None,bounds=None,params0=None):
...
def resultspvd_bay(cube_total,params,disks_f,R_d,dR_d,dic,XX,YY,VV,AA,F,papvds,
...
#                 ax[j].plot(r_model[np.argsort(r_model)],v_model[np.argsort(r_model)],'o',markersize=3.5,alpha=0.7,label=pard)
        ...
        sinpa=np.sin(phi)
        tanpa=np.tan(phi)
        Yslit_l=slit/(2*cospa)+XX*tanpa
        Yslit_u=-slit/(2*cospa)+XX*tanpa
        
        XX_s=XX[(Yslit_l<YY)&(YY<Yslit_u)]
        YY_s=YY[(Yslit_l<YY)&(YY<Yslit_u)]
        VV_s=VV[(Yslit_l<YY)&(YY<Yslit_u)]
        AA_s=AA[(Yslit_l<YY)&(YY<Yslit_u)]
        F_s=F[(Yslit_l<YY)&(YY<Yslit_u)]
        ...
        ax[j].scatter(RR_s*np.sign(-np.cos(PHI_s)),VV_s,marker='.',c=AA_s,norm=LogNorm(),alpha=0.2)
        #ax[j].scatter((RR_s*np.sign(-np.cos(PHI_s)))[F_s],VV_s[F_s],marker='.',c=Ca_res[F_s],alpha=0.2)
